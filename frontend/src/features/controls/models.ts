...
// repo naming) catalog. Support tool/function calling, which the agent graph
// relies on (news_analyst's structured sentiment JSON, lead_architect's
// order proposals). Exact slugs drift as the provider updates its catalog —
// always confirm with "Test API" before relying on one live.
//
// Fireworks support commented out (not deleted) for the submission demo —
// Featherless is the hackathon partner, so it's the only provider shown.
