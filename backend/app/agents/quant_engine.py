...
from app.quant.technical_signals import check_momentum_swing_confluence, check_track1_confluence, check_wheel_put_regime, detect_breakout
...
def run(state: AgentState) -> dict:
    ...
    # see track4_income_wheel.py). Track 5 gets a deliberately thinner
    # 5m/1h confluence check (see technical_signals.check_momentum_swing_confluence's
    # module comment for why it's intentionally looser than Track 1's).
    # Tracks 2/3 keep today's single-timeframe SMA breakout, unchanged.
    ...
    elif state["track"] == "track5_momentum_swing":
        technical_signal = check_momentum_swing_confluence(state.get("bars_5m", []), state.get("bars_1h", []))
    else:
