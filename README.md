...
**LLM validation:** every qualifying setup is checked by an LLM — served by **Featherless AI** running **Kimi K2 Instruct** — acting as lead strategist. This is where the model earns its place: the deterministic gate above can confirm a *pattern* (price moved, trend agrees), but it has no way to know whether that pattern is happening *for a reason a person would trust* — a real catalyst, a scheduled macro release, breaking news that flatly contradicts the direction. The LLM is handed the technical setup plus live news headlines and today's macro calendar, and has to return structured JSON (`verdict`, `sentiment_score`, `confidence_pct`, `thesis`, and its own proposed exit targets) that the strategy layer then clamps to hard platform risk limits. It's a *contradiction* check, not a confirmation requirement — a setup isn't blocked by an absence of supporting news, only by sentiment that actively contradicts the technical direction (gated at ±0.35) or by confidence below 55%. This mirrors how a real discretionary trader treats "no news" — as a green light, not a red flag. Every `thesis` the model writes is stored and shown verbatim in the dashboard's Decision History, approved or rejected — nothing it says is hidden from a judge reviewing the run.

**Try it yourself:** the live app's Controls tab has a masked API-key field for exactly this — paste your own Featherless key and Save to watch the LLM step run live end-to-end (the key is never echoed back by the API once saved, and is stored encrypted at rest where the OS supports it). Without a key, the engine still runs for real — Alpaca connection, the technical gate, and the risk gate all execute live — but a qualifying setup skips the LLM step and is logged as running in demo mode instead, so the rest of the pipeline is never blocked by a missing key.
